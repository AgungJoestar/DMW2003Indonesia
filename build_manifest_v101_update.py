#!/usr/bin/env python3
r"""
DW2003 Indonesian Patcher v1.0.1 manifest builder.

Put these files in ONE folder:
  all_strings.xlsx
  logo.png
  dw2003_patcher_standalone_v101_template.py
  build_manifest_v101.py
  build_exe_v101.bat
...
5. Reads the released v1.0 embedded manifest as the previous translation baseline.
6. Stores English + latest Indonesian + previous v1.0 blobs.
7. Writes dw2003_patcher_standalone_v101_update_built.py without modifying the template.
"""
...
TEMPLATE = HERE / "dw2003_patcher_standalone_v101_update_template.py"
BUILT = HERE / "dw2003_patcher_standalone_v101_update_built.py"
PREVIOUS_BUILT = HERE / "dw2003_patcher_standalone_v100_built.py"
...
def load_manifest_from_built_source(path: Path):
    if not path.is_file():
        raise SystemExit(
            "Previous v1.0 built source tidak ditemukan:\n"
            f"{path}\n\n"
            "Copy dw2003_patcher_standalone_v100_built.py yang dipakai "
            "membuat EXE v1.0 release ke folder builder v1.0.1."
        )

    source = path.read_text(encoding="utf-8")
    m = re.search(
        r"^EMBEDDED_MANIFEST_B85\s*=\s*(['\"])(.*?)\1\s*$",
        source,
        re.MULTILINE,
    )
    if not m:
        raise SystemExit(
            f"Embedded manifest tidak ditemukan di previous build:\n{path}"
        )

    encoded = m.group(2)
    try:
        return pickle.loads(zlib.decompress(base64.b85decode(encoded.encode("ascii"))))
    except Exception as e:
        raise SystemExit(f"Gagal membaca manifest v1.0: {e}")


def previous_target(previous_manifest, resource_key, kind, entry_index, string_index):
    spec = previous_manifest.get("resources", {}).get(resource_key)
    if not spec or spec.get("kind", "pack") != kind:
        return None

    try:
        if kind == "direct":
            pair = spec["strings"][string_index]
        else:
            pair = spec["entries"][entry_index][string_index]
...
def main():
    ...
    previous_manifest = load_manifest_from_built_source(PREVIOUS_BUILT)
    print("Previous release manifest   :", PREVIOUS_BUILT.name)
    print("Previous resources          :", len(previous_manifest.get("resources", {})))
    ...
    for pos, resource_key in enumerate(sorted(grouped), 1):
        ...
        if group_spec["kind"] == "direct":
            ...
            for string_index in sorted(group_spec["strings"]):
                ...
                previous_targets = ()
                if prev is not None and prev != target and prev != expected:
    ...
    print("v1.0 strings upgradeable    :", upgrade_strings)
